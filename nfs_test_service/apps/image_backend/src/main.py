# argocd_deploy_repo/nfs_test_service/apps/image_backend/src/main.py
...
# 환경변수에서 읽어와서 사용하는데 만일 환경변수 값이 존재하지 않으면 기본값 "/mnt/efs/images" 경로를 
...
def get_image(img_id: int):
    ...
    try:
        # img_id에 해당하는 이미지가 어떤 파일명으로 저장되었는지 Select한다
        ...
        # 해당 이미지 data를 읽어올 수 있는 전체 경로를 구성해서 
